	...
	public void validate(final Request<ToolRecord> request, final ToolRecord entity, final Errors errors) {
		...
		if (!errors.hasErrors("sourceType")) {
			if (entity.getSourceType() == null) {
				errors.state(request, entity.getSourceType() != null, "sourceType", "tool-record.requests.error.null");
			} else {
				errors.state(request, entity.getSourceType().equals("Open-Source") || entity.getSourceType().equals("Closed-Source"), "sourceType", "tool-record.requests.error.source");
			}
		}
	}
